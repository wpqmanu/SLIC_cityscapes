...
def predict(random_list,superpixel_data,gt_files,folder_files,current_rule,original_image_files,result_location,is_test_lower_bound,is_use_neighbor,traverse_category_list):
    ...
    for index in random_list:
        ...
        for predicted_index,superpixel_index in enumerate(superpixel_index_set):
            ...
            if current_categorical_labels[2]==6 or current_categorical_labels[2]==7 or current_categorical_labels[2]==9:
                current_categorical_labels[2]=255
            if current_categorical_labels[3]==6 or current_categorical_labels[3]==7:
                current_categorical_labels[3]=255
            if current_categorical_labels[4]==6 or current_categorical_labels[4]==7:
                current_categorical_labels[4]=255
...
def spark_processing(rule_index):
    ...
    superpixel_result_folder='/mnt/scratch/panqu/SLIC/server_combine_all_merged_results_'+dataset+'_use_coarse_train/'
    ...
    traverse_category_list=[6,7,9,255] # you only want to explore several categories (255 means all others)
    random_list=range(0,207)
        ...
        for second_item_in_list in traverse_category_list:
            for third_item_in_list in traverse_category_list:
                for fourth_item_in_list in traverse_category_list:
                    for fifth_item_in_list in traverse_category_list:
                        current_category_list=[first_item_in_list,second_item_in_list,third_item_in_list,fourth_item_in_list,fifth_item_in_list]
    ...
    for index,possible_rule in enumerate(all_possible_rule_list):
        # wall (label 3) and fence (label 4) and pole (label 5)
        if possible_rule[0][2]==6 or possible_rule[0][3]==6 or possible_rule[0][4]==6 or \
           possible_rule[0][2]==7 or possible_rule[0][3]==7 or possible_rule[0][4]==7 or \
           possible_rule[0][2]==9:
    ...
    result_location = os.path.join('/mnt/scratch/panqu/SLIC/prediction_result/four_layers_rule_traverse_category_set_679_use_coarse_train/', dataset,
                                   str(current_rule[0][0])+'_'+str(current_rule[0][1])+'_'+
                                   str(current_rule[0][2])+'_'+str(current_rule[0][3])+'_'+
                                   str(current_rule[0][4])+'_'+str(current_rule[1]))
