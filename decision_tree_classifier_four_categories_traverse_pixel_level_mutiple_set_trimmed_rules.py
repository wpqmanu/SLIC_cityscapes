...
def get_final_rule(how_many_layers,saved_rule_traverse_result, performance_threshold):
    # get result
    ...
    for current_rule in all_rule_result_content:
        ...
        current_rule_performance = current_rule_split[how_many_layers]
...
def predict(random_list,gt_files,folder_files,final_selected_rule_set,original_image_files,result_location,is_test_lower_bound,is_use_neighbor,traverse_category_list):

    img_width=2048
    img_height=1024

    for index in range(len(original_image_files)):
...
if __name__ == '__main__':
    ...
    total_rule_sets=3
    total_layers=4
    traverse_category_list=[[2,3,4,5,255],
                            [6,7,8,9,255],
                            [13,14,15,16,255]] # you only want to explore four categories (255 means all others)
    performance_threshold=[0.75368,0.75368,0.75368]
    saved_rule_traverse_result=['/home/panquwang/adas-segmentation-cityscape/test/rule_traverse_result_file_with_purity.txt',
                                '/home/panquwang/adas-segmentation-cityscape/test/rule_traverse_result_file_with_purity.txt',
                                '/home/panquwang/adas-segmentation-cityscape/test/rule_traverse_result_file_with_purity.txt']
    final_selected_rule_set=[]
    for i in range(len(traverse_category_list)):
        final_selected_rule_set.append(get_final_rule(total_layers,saved_rule_traverse_result[i],performance_threshold[i]))


    # prediction
    result_location = os.path.join('/mnt/scratch/panqu/SLIC/prediction_result/four_layers_'+str(total_rule_sets)+'_rule_sets/', dataset,'all_selected_rules_pixel_level')
    ...
    predict(random_list,gt_files,folder_files,final_selected_rule_set,original_image_files,result_location,is_test_lower_bound,is_use_neighbor,traverse_category_list)
