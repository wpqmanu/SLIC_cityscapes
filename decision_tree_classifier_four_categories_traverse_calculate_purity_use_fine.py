...
def get_stats_345_fine(traverse_category_list,folder_files,gt_files):
    # go through all training files
    C=Counter()
    for index in range(len(gt_files)):
        print index
    ...
    cPickle.dump(C, open(os.path.join('/home/panquwang/SLIC_cityscapes/', 'purity_345_use_fine.dat'), "w+"))

def get_stats_679_fine(traverse_category_list,folder_files,gt_files):
    # go through all training files
    C=Counter()
    for index in range(len(gt_files)):
        print index
    ...
    cPickle.dump(C, open(os.path.join('/home/panquwang/SLIC_cityscapes/', 'purity_679_use_fine.dat'), "w+"))

def get_stats_141516_fine(traverse_category_list,folder_files,gt_files):
    # go through all training files
    C=Counter()
    for index in range(len(gt_files)):
        print index
    ...
    cPickle.dump(C, open(os.path.join('/home/panquwang/SLIC_cityscapes/', 'purity_141516_use_fine.dat'), "w+"))
...
def calculate_purity_with_fine(stats):
    new_list=[]
    for key,value in stats.iteritems():
        current_list=list(key)+[value]
        new_list.append(current_list)
    ...
    new_list_copy=copy.deepcopy(new_list)
    new_list_first_5=np.array([i[:4] for i in new_list])
    for index,item in enumerate(new_list):
        first_five_value=item[:4]
...
if __name__ == '__main__':
    ...
    if is_calculate_purity:
        if fine_or_coarse == 'fine':
            ...
            get_stats_345_fine(traverse_category_list_345,folder_files,gt_files)

            traverse_category_list_679 = [6, 7, 9, 255]  # you only want to explore several categories (255 means all others)
            get_stats_679_fine(traverse_category_list_679,folder_files,gt_files)

            traverse_category_list_141516 = [14, 15, 16, 255]  # you only want to explore several categories (255 means all others)
            get_stats_141516_fine(traverse_category_list_141516,folder_files,gt_files)

    if is_load_purity_result:
        if fine_or_coarse == 'fine':
            purity_345 = cPickle.load(open(os.path.join('/home/panquwang/SLIC_cityscapes/', 'purity_345_use_fine.dat'), "rb"))
            purity_679 = cPickle.load(open(os.path.join('/home/panquwang/SLIC_cityscapes/', 'purity_679_use_fine.dat'), "rb"))
            purity_141516 = cPickle.load(open(os.path.join('/home/panquwang/SLIC_cityscapes/', 'purity_141516_use_fine.dat'), "rb"))

            print "calculating stats..."
            purity_345_final = calculate_purity_with_fine(purity_345)
            purity_679_final = calculate_purity_with_fine(purity_679)
            purity_141516_final = calculate_purity_with_fine(purity_141516)

            cPickle.dump((purity_345_final,purity_679_final,purity_141516_final), open(os.path.join('/home/panquwang/SLIC_cityscapes/', 'purity_final_use_fine.dat'), "w+"))
